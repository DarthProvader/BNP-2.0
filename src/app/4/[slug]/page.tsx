...
export default function ArticleDetailPage({
  ...
  if (!article) {
    return (
      <div
        className={`${courierPrime.variable} ${staatliches.variable} min-h-screen`}
      >
        <style>{classifiedStyles}</style>
        <div className="classified-page min-h-screen flex items-center justify-center">
          <div className="paper-bg p-10 shadow-2xl text-center relative max-w-md">
            <div className="stamp-classified">MISSING</div>
            <h1
              className="text-2xl text-[#1a1a1a] tracking-[0.1em] mb-4 mt-8"
              style={{ fontFamily: "var(--font-staatliches), sans-serif" }}
            >
              FILE NOT FOUND
            </h1>
            <p
              className="text-sm text-[#3a3a3a] tracking-wider mb-6"
              style={{ fontFamily: "var(--font-courier), monospace" }}
            >
              The requested dossier could not be located in Division archives.
              It may have been destroyed or reclassified.
            </p>
            <Link
              href="/4"
              className="text-xs tracking-[0.2em] text-[#cc0000] uppercase hover:underline"
              style={{ fontFamily: "var(--font-courier), monospace" }}
            >
              RETURN TO INDEX
            </Link>
  ...
  const fileNum = getFileNumber(articleIndex);
  const classification = getClassification(articleIndex);
